...
static int load_consumer(mlt_consumer *consumer, mlt_profile profile, int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!strcmp("sdl", argv[i]) || !strcmp("sdl_audio", argv[i])
            || !strcmp("sdl_preview", argv[i]) || !strcmp("sdl_still", argv[i])) {
            fprintf(stderr,
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_WINDOWEVENT:
            if (mlt_properties_get(MLT_CONSUMER_PROPERTIES(consumer), "mlt_service")
                && !strcmp("sdl2",
                           mlt_properties_get(MLT_CONSUMER_PROPERTIES(consumer), "mlt_service")))
...
static void show_usage(char *program_name)
{
    fprintf(
        ...
        "  -progress2                               Display progress along with position on a new "
        "line\n"
